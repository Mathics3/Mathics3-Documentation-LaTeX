#!/bin/bash
# The program create PDF images that can be embedded into the
# Mathics3 manual. In particular the Mathics3 heptatom logo and the
# Mathics3 logo with a shadow that extends a little bit down forward right.


bs=${BASH_SOURCE[0]}
mydir=$(dirname $bs)
cd $mydir
mydir=$(pwd)
DOC_DIR=$(pwd)

if [[ -n $DOCTEST_LATEX_DATA_PCL ]]; then
    LATEX_DIR=$(basename $DOCTEST_LATEX_DATA_PCL)
else
    LATEX_DIR=${mydir}/mathics3_latex
fi
LATEX_IMAGE_DIR=${LATEX_DIR}/images
DOC_IMAGE_DIR=${DOC_DIR}/documentation-src/images

if [[ ! -d "$LATEX_IMAGE_DIR" ]] ; then
    mkdir -p $LATEX_IMAGE_DIR
fi

for filename in $(find documentation-src/images/ -name "*.eps"); do
	pdf="${DOC_IMAGE_DIR}/$(basename "$filename" .eps).pdf"
	epstopdf "$filename"
	mv "$pdf" $LATEX_IMAGE_DIR
done

for filename in ${mydir}/svg/logo-{heptatom,heptatom-Mathics3,Mathics3-nodrop}.svg; do
    inkscape $filename --export-filename="mathics3_latex/static-images/$(basename "$filename" .svg).pdf" --batch-process
done
